...
export default function AuthPage() {
  ...
  const handleSuccess = (role: string = 'CITIZEN') => {
    const r = role.toUpperCase();
    if (r === 'ADMIN') navigate('/admin');
    else if (r === 'ENTERPRISE') navigate('/enterprise');
    else if (r === 'COLLECTOR') navigate('/collector');
  ...
  const titles = {
    login: {
      ...
      subtitle: 'Đăng nhập để tiếp tục quản lý rác thải thông minh',
    },
    register: {
      title: 'Tạo tài khoản mới',
      subtitle: 'Đăng ký để bắt đầu hành trình xanh của bạn',
    },
    'forgot-password': {
      title: 'Khôi phục mật khẩu',
      subtitle: 'Nhập email để nhận hướng dẫn lấy lại mật khẩu',
    },
  };
        ...
        <div className="max-w-lg text-white z-10 space-y-8">
          ...
          >
            <h2 className="text-3xl font-display font-bold leading-tight">
              Tham gia cùng chúng tôi để tạo ra một{' '}
              <span className="text-brand-200">tương lai xanh</span>
            </h2>

            <p className="mt-4 text-lg text-brand-50 leading-relaxed">
              Hơn <span className="font-bold text-white">10,000+</span> người dùng đã tin tưởng
              GreenLoop để quản lý và tái chế rác thải thông minh hơn.
            </p>
          </motion.div>
          ...
          <div className="text-center mb-8">
            ...
            <AnimatePresence mode="wait" initial={false}>
              ...
              >
                ...
                <p className="text-gray-600">{titles[view].subtitle}</p>
              </motion.div>
            </AnimatePresence>
          </div>
            ...
            <AnimatePresence mode="wait" initial={false}>
              {view === 'login' ? (
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.2 }}
                >
                  <RegisterForm onToggleLogin={() => setView('login')} onSuccess={handleSuccess} />
                </motion.div>
              ) : (
                <motion.div
                  key="forgot-password"
