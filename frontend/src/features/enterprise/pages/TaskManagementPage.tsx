...
import type { TaskResponse, TaskStatus, TaskFilters } from '../types';
...
export const TaskManagementPage: React.FC = () => {
    ...
    const loadTasks = useCallback(async (entId?: string) => {
        try {
            setLoading(true);
    ...
    const handleAccept = async (task: TaskResponse) => {
        try {
            await enterpriseService.acceptTask(task.id, enterpriseId);
            loadTasks();
        } catch (err) {
            console.error(err);
        }
    ...
    const handleReject = async () => {
        ...
        try {
            await enterpriseService.rejectTask(selectedTask.id, enterpriseId, rejectReason);
    ...
    const handleAssign = async () => {
        ...
        try {
            await enterpriseService.assignTask({ taskId: selectedTask.id, collectorUserId: collectorId });
    ...
    return (
        <EnterpriseLayout title="Collection Requests" subtitle="Manage and assign waste collection tasks">
            {/* Status Tabs */}
            <div className="bg-white rounded-xl border border-gray-200 p-1.5 flex gap-1 overflow-x-auto shadow-sm">
                {STATUS_TABS.map((tab) => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${statusFilter === tab.value
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-gray-50 border-b border-gray-200">
                                        <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Waste Type</th>
                                        <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Address</th>
                                        <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Citizen</th>
                                        ...
                                        <tr key={task.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className="text-sm text-gray-600 truncate max-w-[200px]">{task.address || '—'}</span>
                                                </div>
                                            </td>
                                            <td className="px-5 py-4">
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 mx-4 max-h-[80vh] overflow-y-auto">
                        ...
                        <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                                <div><p className="text-xs text-gray-400">Status</p><span className={`text-xs px-2.5 py-1 rounded-full font-medium ${getStatusBadge(selectedTask.status)}`}>{selectedTask.status.replace(/_/g, ' ')}</span></div>
                                <div><p className="text-xs text-gray-400">Waste Type</p><p className="text-sm font-medium text-gray-900">{selectedTask.wasteType || '—'}</p></div>
                                <div><p className="text-xs text-gray-400">Citizen</p><p className="text-sm text-gray-700">{selectedTask.citizenName || '—'}</p></div>
                                <div><p className="text-xs text-gray-400">Phone</p><p className="text-sm text-gray-700">{selectedTask.citizenPhone || '—'}</p></div>
                            </div>
                            <div><p className="text-xs text-gray-400">Address</p><p className="text-sm text-gray-700">{selectedTask.address || '—'}</p></div>
                            <div><p className="text-xs text-gray-400">Description</p><p className="text-sm text-gray-700">{selectedTask.description || '—'}</p></div>
                            {selectedTask.rejectionReason && (
                                <div className="bg-red-50 rounded-lg p-3"><p className="text-xs text-red-400">Rejection Reason</p><p className="text-sm text-red-700">{selectedTask.rejectionReason}</p></div>
                            )}
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2 overflow-x-auto">
                                        {selectedTask.imageUrls.map((url, i) => (
                                            <img key={i} src={url} alt={`Report ${i + 1}`} className="w-24 h-24 object-cover rounded-lg border border-gray-200" />
                                        ))}
                                    </div>
                                </div>
                            )}
                            <div className="text-xs text-gray-400 pt-2">Created: {new Date(selectedTask.createdAt).toLocaleString()}</div>
                        </div>
                    </div>
                </div>
            )}
