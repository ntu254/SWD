  ...
  useEffect(() => {
    ...
    wasteReportService
      ...
      .then(areas => {
        ...
        if (areas.length === 1) setSelectedAreaId(areas[0].areaId);
      })
        ...
        {step === 2 && (
          <div className="p-6 space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                  Khu vực thu gom <span className="text-gray-400 font-normal">(tuỳ chọn)</span>
                </label>
                {areasLoading ? (
                  <p className="text-xs text-gray-400 italic">Đang tải khu vực...</p>
                ) : areasError ? (
                  <p className="text-xs text-red-500">
                    Không thể tải khu vực. Bạn vẫn có thể tiếp tục.
                  </p>
                ) : serviceAreas.length === 0 ? (
                  <p className="text-xs text-gray-400 italic">
                    Chưa có khu vực nào được thiết lập. Bạn vẫn có thể tiếp tục.
                  </p>
                ) : (
                  <select
