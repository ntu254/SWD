import { Flame, Medal, Recycle, Trophy, Users, Zap } from 'lucide-react';
import React from 'react';

const IMPACT_STATS = [
  {
    icon: <Recycle size={24} className="text-brand-600" />,
    bg: 'bg-brand-50 border-brand-100',
    iconBg: 'bg-brand-100',
    value: '1,250 tấn',
    label: 'Rác tái chế thành công',
  },
  {
  ...
  },
  {
    ...
    value: '95%',
    label: 'Độ chính xác AI phân loại',
  },
];
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-16">
          {IMPACT_STATS.map((stat, idx) => (
            <div
              key={idx}
              className={`flex items-center gap-4 rounded-2xl border p-5 bg-white/80 backdrop-blur-sm shadow-sm hover:shadow-md transition-shadow ${stat.bg}`}
            >
              <div
          ...
          <div className="bg-white rounded-[2rem] p-8 shadow-xl shadow-brand-100/50 border border-gray-100 relative group hover:border-brand-200 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div className="flex items-center gap-4 mb-8 relative z-10">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm font-medium">
                  Xếp hạng theo điểm phân loại đúng
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3 relative z-10">
              {[
                ...
                <div
                  key={idx}
                  className="flex items-center justify-between p-4 rounded-2xl bg-gray-50 hover:bg-brand-50 border border-transparent hover:border-brand-100 transition-all group/item cursor-default"
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-8 h-8 flex items-center justify-center font-bold rounded-full shadow-sm text-sm ${
                        idx === 0
                  ...
                  <span className="font-display font-bold text-brand-600 bg-white px-4 py-1.5 rounded-full shadow-sm border border-brand-100 group-hover/item:scale-105 transition-transform">
                    {user.points}{' '}
                    <span className="text-xs font-sans font-normal text-gray-500">GP</span>
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-[2rem] p-8 shadow-xl shadow-accent-100/50 border border-gray-100 relative group hover:border-accent-200 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div className="space-y-3 relative z-10">
              {[
                ...
                <div
                  key={idx}
                  className="flex items-center justify-between p-4 rounded-2xl bg-gray-50 hover:bg-accent-50 border border-transparent hover:border-accent-100 transition-all cursor-default"
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-8 h-8 flex items-center justify-center font-bold rounded-full shadow-sm text-sm ${
                  ...
                  <div className="text-right">
                    <span className="block font-bold text-accent-600 text-sm uppercase tracking-wide">
                      Thưởng
                    </span>
                    <span className="text-base font-bold text-gray-700 bg-white px-3 py-1 rounded-lg border border-gray-100 shadow-sm mt-1 inline-block">
                      {col.bonus}
                    </span>
                  </div>
                </div>
        ...
        <p className="text-center text-xs text-gray-400 mt-8">
          Cập nhật hàng tháng · Dữ liệu tháng 3/2026
        </p>
      </div>
    </section>
  );
